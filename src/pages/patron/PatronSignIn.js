...
function PatronSignIn() {
  ...
  const handleSignIn = async (e) => {
    ...
    try {
      const response = await axios.get(`https://springbootlibrarymanagement-production.up.railway.app/patrons/all`);
      ...
      if (response.data) {
        ...
        if (librarianExists) {
          ...
          navigate('/patrondashboard?success=Signed%20in'); // Navigate to the dashboard page
        } else {
  ...
  return (
    <div>
      ...
      <Link to="/patronsignup" className="signup-link">Sign Up</Link>
